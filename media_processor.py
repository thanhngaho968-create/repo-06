...
def classify_media_error(error_str: str) -> str:
    ...
    - 'Need Auth/Cookie (...)': auth, login, bot, 429, cookie, private, members-only
    - 'Error (Video Unavailable)': video unavailable
    - 'Error (Video Removed)': video has been removed
    - 'Error (Copyright / DMCA)': copyright, DMCA claims
    - 'Error (Invalid URL Format)': invalid or malformed URL
    - 'Error (...)': other permanent or descriptive errors
    """
    if not error_str:
...
def handle_single_video(task: Dict[str, Any], temp_dir: str) -> bool:
    ...
    if not os.path.exists(v_path) or os.path.getsize(v_path) == 0:
        ...
        status_label = classify_media_error(err_v)
        if sheet_row:
...
def handle_playlist(task: Dict[str, Any], temp_dir: str) -> bool:
    ...
    parsed = urlparse(url)
    if parsed.scheme not in ("http", "https") or not parsed.netloc:
        err_status = "Error (Invalid URL Format)"
        if sheet_row:
            gsheet_helper.update_media_task_status(sheet_row, status=err_status, progress="Invalid URL format")
        if chat_id and status_msg_id:
            telegram_helper.edit_message(chat_id, status_msg_id, f"❌ <b>Lỗi quét Playlist:</b> <code>Invalid URL format</code>")
        return False
...
def handle_tiktok(task: Dict[str, Any], temp_dir: str) -> bool:
    ...
    if parsed.scheme not in ("http", "https") or not parsed.netloc:
        ...
        if sheet_row:
            gsheet_helper.update_media_task_status(sheet_row, status=err_status, progress="Invalid URL format")
        if chat_id and status_msg_id:
            telegram_helper.edit_message(chat_id, status_msg_id, f"❌ <b>Lỗi tải TikTok:</b> <code>Invalid URL format</code>")
        return False
...
def handle_fb_insta(task: Dict[str, Any], temp_dir: str) -> bool:
    ...
    if parsed.scheme not in ("http", "https") or not parsed.netloc:
        ...
        if sheet_row:
            gsheet_helper.update_media_task_status(sheet_row, status=err_status, progress="Invalid URL format")
        if chat_id and status_msg_id:
            telegram_helper.edit_message(chat_id, status_msg_id, f"❌ <b>Lỗi tải media:</b> <code>Invalid URL format</code>")
        return False
